...
public class Menu extends KeyAdapter{
	...
	void radioButtons(JRadioButton radiobutton, int sizeX, int sizeY, int corY, int corX) {
		Font font = new Font("Verdana", Font.ITALIC, 20);
		radiobutton.setFont(font);
		radiobutton.setSize(sizeX, sizeY);
		radiobutton.setLocation(corX, corY);
		radiobutton.setBackground(Color.BLUE);
		radiobutton.setForeground(Color.RED);
	}

	void TextField(JTextField textfield, int corY, int corX) {
		Font font = new Font("Verdana", Font.BOLD, 17);
		textfield.setFont(font);
		textfield.setLocation(corX, corY);
	...
	void Start(final JFrame closeFrame, JButton button, final JFrame Menu, 
			final int speed, final int speed_paddle){
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Two Players 
				Menu menu = new Menu();
				PongPanelTwoPlayer pongPanelTwo = new PongPanelTwoPlayer(speed, speed_paddle, 2); 
